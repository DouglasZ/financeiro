package com.br.financeiro.api.storage;
...
public class S3
{
	...
	{
		Assert.notNull( amazonS3, "Não foi possível acessar o servidor de arquivos." );
		...
		final String nomeUnico = gerarNomeUnico( arquivo.getOriginalFilename() );
	...
	{
		// Para caso for usar o Amazon S3. O "\\\\" representa que, não importa se for http ou https
		// return "\\\\" + property.getS3().getBucket() + ".s3.amazonaws.com/" + objeto;

		return property.getS3().getEndPoint() + "/" + property.getS3().getBucket() + "/" + objeto + "?noAuth=true";
	}
	...
	{
		// Removemos a Tag do arquivo, ou seja, ele deixa de ser um arquivo temporário
		...
		// Salvamos o arquivo como permanente
